 ...
 color:red;	
			  ...
			  <div id="studentList" style="display:none">
			       ...
			       <div style="float:left;">
					  ...
					  <input type="submit"  name="exam_appearence" id="exam_appearence" value="Exam Appearance" class="btn btn-primary" formtarget="_blank">
						...
						<input type="submit" name="save"  id="save" value="Register" class="btn btn-primary"  />
				
				</div>
						<table id="example" class="table table-bordered table-hover">
								<!--<thead>
								<tr>
								    <th rowspan="2"><input type="checkbox"  id="select_all">Select All</th>
  ...
  $(document).ready(function() {
	...
	function getStudentList()
	{
		...
		$.ajax({
			type:'POST',
			url:'<?php echo base_url();?>course/getCapStudentList',
			data: $form.serialize(),
			success: function(data){
				//alert(data); 
				 $('#example').html(data);
...
$('.checkbox').change(function(){ 
	...
	</script><?php $this->load->view('admin/helper/footer');?> 