 ...
 color:red;	
	   ...
	   <div class="box box-primary">
            ...
            <form role="form" name="ug_marks_upload_view" id="ug_marks_upload_view" method="post" action="" enctype="multipart/form-data">
			  ...
			  <label for="course-group">Items to be Import<span style="color:red;font-weight: bold;">*</span></label>
			   <select class="form-control" name="marks_type" id="marks_type" onclick="getStudentAssignedMarksPG();">
  ...
  <script type="text/javascript">
	...
	function savePGInternalMarks()
	{
		...
		$.ajax({
			type:'POST',
			url:'<?php echo base_url();?>markspg/savePGMULInternalMarks',
			data: $form.serialize(),
			success: function(data){
	...
	{   var uploadType=$('#marks_type').val();
		...
		$.ajax({
			type:'POST',
			url:'<?php echo base_url();?>markspg/getStudentAssignedMarksPG',
			data: $form.serialize(),
			success: function(data){
				//alert(data); return false;
			$('#trlist').empty();	
