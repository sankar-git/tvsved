	    ...
	    if(!empty($this->input->post('report_card')))
		 {  
								 ...
								 {  
									 ...
									 // p($gradeval); 
									  // $data['sum_total'] = $internal_sum;
									   ...
									   $result['theory_internal']=$subjectVal->theory_internal;
									   $result['theory_paper1']=$subjectVal->theory_paper1;
									   $result['theory_paper2']=$subjectVal->theory_paper2;
									   $result['sum_internal_practical']=$subjectVal->sum_internal_practical;
									   $result['practical_internal']=$subjectVal->practical_internal;
									   $result['theory_external']=$subjectVal->theory_external;
		   ...
		   //generate the PDF from the given html
						...
						$subjectList = $this->Result_model->get_student_marks_by_id($stuData->user_id,$semester_id);
						     ...
						     $list['month_year']  =$monthYrr;
								 ...
								 {  
									   ...
									   $data['theory_internal1']    			= $subjectVal->theory_internal1;
									   $data['theory_internal2']    			= $subjectVal->theory_internal2;
									   $data['theory_internal3']    			= $subjectVal->theory_internal3;
									   $data['theory_internal']    				= $subjectVal->theory_internal;
									   $data['theory_paper1']    				= $subjectVal->theory_paper1;
									   $data['theory_paper2']    				= $subjectVal->theory_paper2;
									   ...
									   $data['student_id']                      =$stuData->user_id;
									  
									   //subjectwise status of pass or fail of bvsc result
										...
										if($sum_interna_passcondition30 >=30 && $sum_practical_pass_condition20 >=20)
										{
									   ...
									   $roundpercent = number_format($percentval,2);
									  //p($roundpercent); exit;
									   if($roundpercent >=50){ $result_status='PASS'; //subject wise pass fail
									   }
									  
									   else{ $result_status='FAIL';  //subject wise pass fail
									   }
									   $gradeval = $roundpercent/10;
									   $gradepercent = number_format($gradeval, 2);
									   $creditpoint = $gradepercent*$total_credits;
									   ...
									   $result['external_sum']=$subjectVal->external_sum;
									   $result['sum_internal']=$sum_internal;
									   $result['pass_condition1']=$sum_interna_passcondition30;
									   $result['pass_condition2']=$sum_practical_pass_condition20;
									   $result['ncc_status']=$subjectVal->ncc_status;
									   $result['course_group_id']=$subjectVal->course_group_id;
									   $result['passfail_status']=$passfail_status;
									   $result['percentval']=$roundpercent;
			...
			//load the view and saved it into $html variable
			$html=$this->load->view('admin/report/student_bvsc_result', $data, true);
			//$html=$this->load->view('admin/report/student_result_view_new', $data, true);
			// print_r($html); exit;
			//this the the PDF filename that user will get to download
			$pdfFilePath = "student_result_view.pdf";
	         ...
	         //echo "hello"; exit;
				...
				if($degree_id==1){
					//echo "hello"; exit;
						...
						$resultList = $this->Result_model->get_deflicit_fail_list($student_id,$semester_id);
						//p($resultList);exit;
								 ...
								 {  
								   ...
								   $studentInfo = $this->Result_model->get_student_info($resultVal->student_id);
									...
									$data['creditval']= $resultVal->creditval;
									$data['credithour']= $resultVal->credithour;
									$data['course_title']= $resultVal->course_code;
									
									$dataList[] = $data;  
								 }    
								 
						 $data['deflicit_student_list']=$dataList;;
