<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	class payment_lib{
		...
		function studentlist($userid,$month,$year,$class_id)
		{
			...
			$wherecls=" where fees_user_class.class_id = ".$class_id." and year(fees_user_class.updated_date) = ".$year.$where;
		...
		if(count($data['user'])>0)
		{
			foreach($data['user'] as $row)
			{
				...
				$totfees=($row->class_fees + $row->specialfees + $row->session_charge + $row->security_deposit ) - $row->concession ;
				$row->totfees=$totfees;
		...
		function user_paymentlist($name,$registration,$userid,$month,$lastmonth,$year,$class_id,$role_id,$class_fees)
		{
			...
			{				
				...
				$paylist[$count]['payable']=($totfees +$paylist[$count]['latefine'])- $paid;	
				...
				foreach($payment as $row)
				{
					...
					$paylist[$rowcount]['payable']=($totfees + $paylist[$count]['latefine'])-$paid;					
		...
		{					
			$table.= '<tr>';
				...
				$table.= '<td>'.number_format(($rows['totfees']),2).'</td>';
		...
		function yearlydue($class_id,$year,$month,$lastmonth)
		{
			...
			if(count($data['user'])>0)
			{
				foreach($data['user'] as $row)
				{
					...
					$row->name.=' '.$row->last_name;	
					$row->session_charge=$this->session_charge($row->user_id,'','',$class_id);
					$row->security_deposit=$this->security_deposit($row->user_id,'','',$class_id);							
					...
					$row->yearly_fees=(($classfees + $row->charges + $row->session_charge + $row->security_deposit) - $row->concession);
					$row->latefine=0;
