(function ($) {
    ...
    var render = {
        ...
        header: function() {
            var header = $("<div></div>").addClass("header hidden");
        ...
        timeslots: function(settings) {
            ...
            for (i = 0; i < hours.length; i++) {
                var hour = $("<div></div>").addClass("hour").text(hours[i].replace('AM',''));
        ...
        for (i = 0; i < rsvns.length; i++) {
            if (rsvns[i].date == date) {
                ...
                if ((start >= firstHour && start <= lastHour) && (end >= firstHour && end <= lastHour) && (rsvns[i].row <= settings.items.length)) {
                    ...
                    var $rsvn = $("<div>"+rsvns[i].subject+"<span class='hidden'>"+rsvns[i].id+"</span></div>"),
